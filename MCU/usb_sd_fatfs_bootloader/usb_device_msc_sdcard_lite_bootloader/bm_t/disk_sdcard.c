...
FRESULT USB_DeviceMscApp(void)
{
	...
	if(wrFlag == 1)
	{
		SDMMCHOST_Delay(300); //because When put file into the disk, the command write10 will excute twice, so when first write has done, wait 300ms to make second write command excute
		wrFlag = 0;
		PRINTF("file mod\r\n");
